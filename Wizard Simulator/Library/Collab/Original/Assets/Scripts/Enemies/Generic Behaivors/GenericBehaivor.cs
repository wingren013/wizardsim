...
public class GenericBehaivor : Enemy_Base {
   ...
   public void SpellCast(string spellstring, Transform rot)
    {
        Destroy(generator.generatedSpell);
        generator.clearText();
        string[] spelltext = spellstring.Split(' ');
        foreach (string word in spelltext)
        {
            generator.AddText(word);
        }
        generator.translateAndMakeSpell();
        GameObject spell = Instantiate(generator.generatedSpell, spellgun.transform.position, rot.rotation) as GameObject;
        Spell.SpellType type = spell.GetComponent<Spell>().spelltype;
        Vector3 distance = Vector3.zero;

        switch (type)
        {
            case Spell.SpellType.BEAM:
                distance = Vector3.zero;
                spell.transform.parent = spellgun.transform;
                spell.transform.localRotation.eulerAngles.Set(0, 0, 0);
                spell.GetComponent<Ray>().myLayerMask = (1 << LayerMask.NameToLayer("Player") | 1 << LayerMask.NameToLayer("Default"));
                spell.SetActive(true);
                break;

            case Spell.SpellType.ORB:
