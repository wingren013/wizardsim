...
public class Enemy_Base : WizSimBehavior {
	...
	public bool CheckCover()
	{
		...
		{
           // Debug.Log("CheckCover():" + hit.collider.tag);
	...
	{
		//Debug.Log("Raycasting!");
		Debug.DrawRay(gameObject.transform.position + raycastOffset, target.transform.position - (gameObject.transform.position + raycastOffset));
		if (Physics.Raycast(gameObject.transform.position + raycastOffset, target.transform.position - (gameObject.transform.position + raycastOffset), out hit))
		{
			//Debug.Log(hit.collider.gameObject.name);
			return (hit.collider.gameObject.tag != "Player");
		}
	...
	public bool CheckCover(Vector3 position, Vector3 raycastOffset)
	{
		//Debug.Log("Raycasting!");
		Debug.DrawRay(gameObject.transform.position + raycastOffset, target.transform.position - (gameObject.transform.position + raycastOffset));
		if (Physics.Raycast(position + raycastOffset, target.transform.position - (gameObject.transform.position + raycastOffset), out hit))
		{
			//Debug.Log(hit.collider.gameObject.name);
			return (hit.collider.gameObject.tag != "Player");
		}
	...
	public bool FindCover()
	{
		...
		foreach (Collider obstacle in cover)
		{
			...
			{
				//80% chance to choose any piece of cover
				if (Random.value > 0.2)
				{
    ...
    //Cast at player
    public void CastAtPlayer(string spellstring)
    {
        spellgun.transform.LookAt(target.transform);
        SpellCast(spellstring, spellgun);
    }
	...
	{
		gameObject.transform.rotation = Quaternion.RotateTowards(gameObject.transform.rotation, Quaternion.LookRotation(player.transform.position), rotspeed);
	}
	public void FacePlayer(float speed)
	{
		agent.updateRotation = false;
		Debug.Log("facing");
		Debug.Log(gameObject.transform.rotation);
		Debug.Log(gameObject.name);
		gameObject.transform.rotation = Quaternion.RotateTowards(gameObject.transform.rotation, Quaternion.LookRotation(player.transform.position), speed);
		Debug.Log(gameObject.transform.rotation);
	}
}
